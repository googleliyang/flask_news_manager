...
def login():
    ...
    # 缓存用户管理员的身份
    # 把passport/views.py文件中的用户退出代码，添加退出管理员session.pop('is_admin',None)
...
def user_count():
    ...
    --- 查询所有非管理员的所有人
    2、月新注册的人数
    --- 2.1查询所有非管理员的、每月1日0时0分0秒后新注册的用户，create_time
    --- 2.2标准日期格式：2019-01-23
    --- 2.3t = time.localtime()
    --- 2.4tm_year=2019, tm_mon=1, tm_mday=23,
    --- 2.5每月的1号 = '%d-%02d-01' % (t.tm_year,t.tm_mon)
    --- 2.6把日期字符串转成日期对象
    --- 2.7比较日期，拿到每月的1号，用户的注册时间大于每月1号0时0分0秒

    3、日新注册人数
    --- 3.1查询所有非管理员的、每天的0时0分0秒后新注册的用户
    --- 3.2每天的开始时间 = '%d-%02d-%02d' % (t.tm_year,t.tm_mon,t.tm_mday)
    --- 3.3比较日期，拿到每天的时间，用户的注册时间大于每天的开始时间
    4、用户的活跃人数，登录时间last_login
    5、用户的活跃日期
    --- 统计的是每天的活跃人数和日期，列表存储每天的数据
    --- timedelta模块，可以从当前日期往前推
    --- 获取当前日期字符串，转成日期对象
    --- 获取每天的开始时间和结束时间，获取每天的活跃人数和日期
    --- 当前日期 - timedelta(day)
